...
if (!isset($_GET['patientID'])) {
    die("Patient user ID required");
}
$patientUserID = intval($_GET['patientID']);
...
$stmt->execute([$doctorUserID, $patientUserID, $patientUserID, $doctorUserID]);
...
<body>
    <div class="chat-container">
        <div class="chat-header">
            ...
            <h5 class="doctor-name"><?= htmlspecialchars($patientName) ?></h5>
        </div>
    ...
    <script>
        ...
        function loadMessages() {
            $.get('get_messages.php', { chatRoomID, lastMessageID }, function(data) {
                data.forEach(msg => {
                    const isOwnMessage = msg.SenderID == doctorUserID;
                    const msgDiv = document.createElement('div');
                    msgDiv.className = `message ${isOwnMessage ? 'patient-message' : 'doctor-message'}`;
                    msgDiv.innerHTML = `
