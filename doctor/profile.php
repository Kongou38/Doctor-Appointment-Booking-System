...
function getDoctorById(int $doctorId): ?array {
    $conn = null;
    try {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_profile') {
    ...
    try {
        ...
        $doctorIDToUpdate = $_POST['doctor_id'] ?? 0;
        $doctorUserIDToUpdate = $_POST['user_id'] ?? 0;
        ...
        $stmt->bindParam(':userId', $doctorUserIDToUpdate, PDO::PARAM_INT);
        $stmt->execute();
        ...
        error_log("Doctor profile update PDO error: " . $e->getMessage());
    } catch (Exception $e) {
        if ($conn) $conn->rollback();
...
$doctorDetails = getDoctorById($doctorID);

if (!$doctorDetails) {
    error_log("Critical error: Doctor info not found for DoctorID: " . $doctorID);
...
<body>
    <div class="profile-container">
        <div class="profile-header">
            <h4><i class="fas fa-user-circle me-2"></i>Doctor Profile</h4>
        </div>
        ...
        <form method="POST" action="profile.php">
            ...
            <input type="hidden" name="doctor_id" value="<?= htmlspecialchars($doctorDetails['DoctorID']) ?>">
            <input type="hidden" name="user_id" value="<?= htmlspecialchars($doctorDetails['DoctorUserID']) ?>">

            <div class="profile-field">
                <div class="field-label">Name</div>
                <div class="field-value-display" id="nameDisplay"><?= htmlspecialchars($doctorDetails['Name']) ?></div>
                <input type="text" class="form-control field-value-edit hidden" id="nameInput" name="name" value="<?= htmlspecialchars($doctorDetails['Name']) ?>">
            </div>
            
            <div class="profile-field">
                <div class="field-label">IC. Number</div>
                <div class="field-value-display" id="icNumberDisplay"><?= htmlspecialchars($doctorDetails['ICNumber']) ?></div>
                <input type="text" class="form-control field-value-edit hidden" id="icNumberInput" name="ic_number" value="<?= htmlspecialchars($doctorDetails['ICNumber']) ?>" readonly>
            </div>
            
            <div class="profile-field">
                <div class="field-label">Contact</div>
                <div class="field-value-display" id="contactNumberDisplay"><?= htmlspecialchars($doctorDetails['ContactNumber']) ?></div>
                <input type="tel" class="form-control field-value-edit hidden" id="contactNumberInput" name="contact_number" value="<?= htmlspecialchars($doctorDetails['ContactNumber']) ?>">
            </div>
            
            <div class="profile-field">
                <div class="field-label">Email</div>
                <div class="field-value-display" id="emailDisplay"><?= htmlspecialchars($doctorDetails['Email']) ?></div>
                <input type="email" class="form-control field-value-edit hidden" id="emailInput" name="email" value="<?= htmlspecialchars($doctorDetails['Email']) ?>">
            </div>
