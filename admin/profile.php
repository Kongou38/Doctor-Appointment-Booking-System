...
function getAdminByUserId(int $userId): ?array {
    $conn = null;
    try {
        ...
        $stmt = $conn->prepare("
            SELECT 
                ...
                su.ContactNumber
            FROM Admin a
            JOIN SystemUser su ON a.UserID = su.UserID
        ...
        error_log("Error fetching Admin by UserID: " . $e->getMessage());
        return null;
    } finally {
        $conn = null;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_profile') {
    ...
    try {
        ...
        $adminIdToUpdate = $_POST['admin_id'] ?? 0;
        ...
        error_log("Admin profile update PDO error: " . $e->getMessage());
    } catch (Exception $e) {
        if ($conn) $conn->rollback();
...
$adminDetails = getAdminByUserId($userId);

if (!$adminDetails) {
    error_log("Critical error: Admin info not found for UserID: " . $userId);
...
<body>
    <div class="profile-container">
        ...
        <form method="POST" action="profile.php">
            ...
            <input type="hidden" name="admin_id" value="<?= htmlspecialchars($adminDetails['AdminID']) ?>">
            <input type="hidden" name="user_id" value="<?= htmlspecialchars($adminDetails['UserID']) ?>">

            <div class="profile-field">
                <div class="field-label">Name</div>
                <div class="field-value-display" id="nameDisplay"><?= htmlspecialchars($adminDetails['Name']) ?></div>
                <input type="text" class="form-control field-value-edit hidden" id="nameInput" name="name" value="<?= htmlspecialchars($adminDetails['Name']) ?>">
            </div>
            
            <div class="profile-field">
                <div class="field-label">IC. Number</div>
                <div class="field-value-display" id="icNumberDisplay"><?= htmlspecialchars($adminDetails['ICNumber']) ?></div>
                <input type="text" class="form-control field-value-edit hidden" id="icNumberInput" name="ic_number" value="<?= htmlspecialchars($adminDetails['ICNumber']) ?>" readonly>
            </div>
            
            <div class="profile-field">
                <div class="field-label">Contact</div>
                <div class="field-value-display" id="contactNumberDisplay"><?= htmlspecialchars($adminDetails['ContactNumber']) ?></div>
                <input type="tel" class="form-control field-value-edit hidden" id="contactNumberInput" name="contact_number" value="<?= htmlspecialchars($adminDetails['ContactNumber']) ?>">
            </div>
            
            <div class="profile-field">
                <div class="field-label">Email</div>
                <div class="field-value-display" id="emailDisplay"><?= htmlspecialchars($adminDetails['Email']) ?></div>
                <input type="email" class="form-control field-value-edit hidden" id="emailInput" name="email" value="<?= htmlspecialchars($adminDetails['Email']) ?>">
            </div>
